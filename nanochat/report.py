...
def get_gpu_info():
    """Get GPU information."""
    if not any([torch.cuda.is_available(), torch.xpu.is_available()]):
        return {"available": False}

    num_devices = ezpz.get_gpus_per_node()
    # num_devices = torch.cuda.device_count()
    info = {
        "available": True,
        "gpu_type": "cuda" if torch.cuda.is_available() else "xpu",
        "count": num_devices,
        "names": [],
        "memory_gb": [],
    }
    ...
    for i in range(num_devices):
        ...
        props = ezpz.get_device_properties(i)
        info["names"].append(getattr(props, "name", "unknown"))
        info["memory_gb"].append(getattr(props, "total_memory", 0) / (1024**3))

    # Get CUDA version
    if torch.cuda.is_available():
        info["cuda_version"] = torch.version.cuda or "unknown"  # type:ignore
    elif torch.xpu.is_available():
        info["xpu_version"] = torch.version.xpu or "unknown"  # type:ignore
...
"""
    ...
    if gpu_info.get("available"):
        gpu_type = gpu_info["gpu_type"].upper()
        gpu_names = ", ".join(set(gpu_info["names"]))
        total_vram = sum(gpu_info["memory_gb"])
        header += f"""- {gpu_type} GPUs: {gpu_info["count"]}x {gpu_names}
- GPU Memory: {total_vram:.1f} GB total
- CUDA Version: {gpu_info.get("cuda_version", None)}
- XPU Version: {gpu_info.get("xpu_version", None)}
"""
    else:
...
def get_report():
    ...
    if ezpz.get_rank() == 0:
        report_dir = os.path.join(str(get_base_dir()), "report")
