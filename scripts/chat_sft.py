...
# Compute init
device_type = ezpz.dist.get_torch_device_type() if device_type == "" else device_type
...
for step in range(num_iterations):
    ...
    if last_step or step % eval_every == 0:
        ...
        if ddp:
            dist.all_reduce(val_loss, op=dist.ReduceOp.SUM)  # average over ranks
        val_loss = val_loss.item() / ezpz.dist.get_world_size()
